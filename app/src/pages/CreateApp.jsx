...
import { PlusCircle, ShieldCheck, Copy, Check, Rocket, Cpu, Globe, Key, ArrowRight } from 'lucide-react';
...
export default function CreateApp() {
  ...
  const handleCreate = async (e) => {
    ...
    // Simulated API call to your Gateway Service
    setTimeout(() => {
      const mockKey = `me_live_${Math.random().toString(36).substring(2, 15)}`;
    ...
    <div className="max-w-4xl mx-auto space-y-10 animate-in fade-in slide-in-from-bottom-8 duration-700">
      ...
      <div className="text-center space-y-4">
        ...
        <h1 className={`text-4xl font-black tracking-tight ${isDarkMode ? "text-white" : "text-gray-900"}`}>
          Deploy New <span className="text-transparent bg-clip-text bg-gradient-to-r from-indigo-500 to-purple-500">Memory Instance</span>
        </h1>
        <p className={`max-w-lg mx-auto text-lg ${isDarkMode ? "text-gray-400" : "text-gray-500"}`}>
          Connect your external applications to our high-speed retrieval bridge.
        </p>
      </div>

      {!generatedKey ? (
        <div className={`relative group p-[1px] rounded-[2.5rem] bg-gradient-to-b ${
            ...
            <div className="grid md:grid-cols-2 gap-8">
              ...
              <div className="space-y-6">
                <div className="space-y-2">
                  <label className={`flex items-center gap-2 text-sm font-bold uppercase tracking-widest ${isDarkMode ? "text-gray-500" : "text-gray-400"}`}>
              ...
              <div className={`p-6 rounded-3xl border flex flex-col justify-center space-y-4 ${
                isDarkMode ? "bg-gray-900/50 border-gray-800" : "bg-indigo-50/50 border-indigo-100"
              }`}>
                <h3 className={`font-bold flex items-center gap-2 ${isDarkMode ? "text-indigo-400" : "text-indigo-700"}`}>
                  <ShieldCheck size={18} /> Instant Provisioning
                </h3>
                <ul className={`text-sm space-y-3 ${isDarkMode ? "text-gray-400" : "text-gray-600"}`}>
                  <li className="flex items-start gap-2">• Auto-scales with your request volume.</li>
                  <li className="flex items-start gap-2">• Secured with SHA-256 encryption.</li>
                  <li className="flex items-start gap-2">• Ready for Production immediately.</li>
                </ul>
              </div>
            </div>
        ...
        <div className={`p-1 rounded-[2.5rem] bg-gradient-to-r from-emerald-500 to-indigo-500 animate-in zoom-in-95 duration-500 shadow-2xl`}>
          <div className={`p-10 rounded-[2.45rem] flex flex-col items-center text-center space-y-6 ${isDarkMode ? "bg-gray-950" : "bg-white"}`}>
            ...
            <div className="space-y-2">
              ...
              <p className={isDarkMode ? "text-gray-400" : "text-gray-500"}>
                Your application <b>{appName}</b> is now live. Use the key below in your header.
              </p>
            </div>

            <div className={`group relative w-full max-w-xl p-1 rounded-2xl transition-all ${
              isDarkMode ? "bg-gray-900" : "bg-gray-100"
            }`}>
              <div className="flex items-center gap-4 px-6 py-5 font-mono text-lg">
                <Key size={20} className="text-indigo-500" />
                <span className={`flex-1 truncate ${isDarkMode ? "text-emerald-400" : "text-emerald-600"}`}>
                  {generatedKey}
                </span>
                <button 
                  onClick={copyToClipboard}
                  className={`p-3 rounded-xl transition-all ${
                    copied ? "bg-emerald-500 text-white" : "hover:bg-indigo-500/10 text-indigo-500"
                  }`}
                >
